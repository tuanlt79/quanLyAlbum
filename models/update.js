export class Update{
    ...
    xoaAlbum(linkAnh) {
        ...
        let index = this.danhSachAlbum.findIndex((item) => item.linkAnh === linkAnh)
        if (index !== -1) {
            this.danhSachAlbum.splice(1, index);   
        };
     };
    suaAlbum(linkAnh) {
        let index = this.danhSachAlbum.findIndex(item => item.linkAnh === linkAnh)
        ...
        document.getElementById('theLoai').value = albumSua.linkAnh;
        document.getElementById('tenAlbum').value = albumSua.tenAlbum;
     };
    capNhatAlbum(linkAnh) {
        
        let index = this.danhSachAlbum.findIndex(item => item.linkAnh === linkAnh)
        danhSachAlbum[index] = linkAnh;
     };
    saveAlbum() {
